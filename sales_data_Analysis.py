...
def main():
    ...
    bathing_soap = [int(sales_data[i][4]) for i in range(1,len(sales_data))]
    shampoo = [int(sales_data[i][5]) for i in range(1,len(sales_data))]
    moisturizer = [int(sales_data[i][6]) for i in range(1,len(sales_data))]
    ...
    plt.plot(month_number, bathing_soap, label='Bathing Soap Sales Data', marker='o')
    plt.plot(month_number, shampoo, label='Shampoo Sales Data', marker='o')
